...
struct ws_server_callbacks {
	// Parsed JSON object of a text frame; only well-formed JSON objects are
	// delivered. The reference is valid only for the duration of the call.
	std::function<void(uint64_t conn, const nyan_json &msg)> on_text;
