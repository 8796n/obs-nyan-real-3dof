...
void run_sensor_api_session(device_manager *f, uint32_t &seen_epoch)
{
	...
	vec3d g_latest = {};
	bool live = false;
	const uint64_t probe_start = nyan_now_ns();
	while (!f->stop.load(std::memory_order_relaxed) &&
	       nyan_now_ns() - probe_start < 1500000000ULL) {
	...
	if (serial != INVALID_HANDLE_VALUE &&
		...
		for (char *p = raw; *p; ++p)
			if (!std::isprint(static_cast<unsigned char>(*p)))
				*p = '.';
		nyan_log(NYAN_LOG_INFO, "[obs-nyan-real-3dof] getbright raw reply: '%s'",
		     raw);
	}
	...
	while (!f->stop.load(std::memory_order_relaxed) &&
		...
		if (nyan_now_ns() - last_rx_ns > 3000000000ULL) {
			if (f->debug_log.load(std::memory_order_relaxed))
				nyan_log(NYAN_LOG_WARNING,
	...
	publish_pose(f, false);
	if (f->debug_log.load(std::memory_order_relaxed))
		nyan_log(NYAN_LOG_WARNING, "[obs-nyan-real-3dof] Sensor API disconnected");
	seen_epoch = f->reconnect_epoch.load(std::memory_order_relaxed);
