...
void run_one_bridge_tcp_session(device_manager *f, uint32_t &seen_epoch)
{
	winsock_scope ws;
	if (!ws.ok()) {
		nyan_log(NYAN_LOG_ERROR, "[obs-nyan-real-3dof] WSAStartup failed");
	...
	if (s == INVALID_SOCKET) {
		...
		if (f->debug_log.load(std::memory_order_relaxed))
			nyan_log(NYAN_LOG_WARNING, "[obs-nyan-real-3dof] TCP connect failed: %s:%d",
			     ip.c_str(), port);
		std::this_thread::sleep_for(std::chrono::milliseconds(500));
		return;
	}
	...
	publish_pose(f, true);
	if (f->debug_log.load(std::memory_order_relaxed)) {
		nyan_log(NYAN_LOG_INFO, "[obs-nyan-real-3dof] TCP connected: %s:%d%s%s",
	...
	uint64_t last_eye_poll_ns = nyan_now_ns();

	std::vector<uint8_t> stash;
	stash.reserve(65536);
	std::vector<uint8_t> buf(16384);
	rate_log_state rate_log;
	uint64_t last_record_ns = nyan_now_ns();
	...
	while (!f->stop.load(std::memory_order_relaxed) &&
		...
		if (nyan_now_ns() - last_record_ns > 3000000000ULL) {
			if (f->debug_log.load(std::memory_order_relaxed))
	...
	publish_pose(f, false);
	if (f->debug_log.load(std::memory_order_relaxed))
		nyan_log(NYAN_LOG_WARNING, "[obs-nyan-real-3dof] TCP disconnected");
	seen_epoch = f->reconnect_epoch.load(std::memory_order_relaxed);
