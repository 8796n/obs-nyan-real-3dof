...
// tracker it feeds, and the dock-facing setters. OBS-independent; the obs_data
// settings load/save lives in the OBS-side device_settings_obs glue.
#pragma once
...
// the sbs_output setting (auto/on/off) combined with the glasses display mode.
// Shared by the renderer, the dock's IPD row and the phone remote so the SBS
// gate is computed in exactly one place.
