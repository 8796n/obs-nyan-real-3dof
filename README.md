...
2. 表示された QR コードをスマホのカメラで読み取り、開いたページをそのまま使います
   （`ホーム画面に追加` するとフルスクリーンのアプリ感覚になります）。

ページの構成は、中央がタッチパッド（1 本指で移動、タップで左クリック、2 本指で
スクロール、2 本指タップで右クリック）、下部が押している間ホールドされる左右クリック
ボタン（ドラッグ用）、上部が `センター` / `再較正`、右端が `スクリーン距離` ホイール
