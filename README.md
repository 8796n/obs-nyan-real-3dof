...
HID で判別した AR グラス（XREAL / RayNeo）の IMU データを読み取り、OBS のソースを
...
HID device detection (XREAL / RayNeo)
  ...
  -> Air-family / RayNeo Air-family: HID 入力レポートの IMU/MAG ストリーム
  -> ジャイロ + 加速度による姿勢トラッキング
  -> 任意で MAG ヨー補正
  -> OBS 内での仮想スクリーンソース描画
```
...
1. 対応するグラス（XREAL / RayNeo）を USB で接続し、HID デバイスとして認識させます。
2. OBS を起動します。
...
**ゴール:** 複数のディスプレイ（物理でも、Parsec VDD などで作った仮想でも）を 1 枚の
大きな仮想スクリーンとして AR グラスに固定表示し、見たい画面の方へ首を向けて見る。
...
- 並べたいディスプレイが Windows 上でアクティブになっている
...
OBS ソースにまとめます。Parsec VDD などで増やした仮想ディスプレイを、Windows の
...
機種（XREAL One / One Pro / 1S / ROG XREAL R1 / Air / Air 2、RayNeo Air 系）は HID で
