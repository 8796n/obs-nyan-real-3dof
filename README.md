...
HID で判別した AR グラス（XREAL / RayNeo / EPSON MOVERIO / Rokid / VITURE / Nreal Light）の IMU データを読み取り、OBS のソースを
...
HID device detection (XREAL / RayNeo / EPSON MOVERIO / Rokid / VITURE / Nreal Light)
  ...
  -> VITURE: HID 入力レポートの本体フュージョン済み姿勢ストリーム
  ...
  -> スマホリモコン (同一Wi-FiのスマホがQRコードを読むだけでタッチパッド + センター/再較正/スクリーン距離リモコンに)
```
...
1. 対応するグラス（XREAL / RayNeo / EPSON MOVERIO / Rokid / VITURE / Nreal Light）を USB で接続し、HID デバイスとして認識させます。
2. OBS を起動します。
...
- 特定のアプリや動画だけ黒く映る → DRM 保護コンテンツ（Netflix・Amazon Prime
  Video・Disney+ など）です。Windows の出力保護により**仕様上キャプチャできません**
  （後述「Display Wall」の DRM に関する注意を参照。回避手段はありません）
...
> **DRM 保護コンテンツは映りません（黒画面）。** Netflix・Amazon Prime Video・
> Disney+ などの保護された再生は、Windows の出力保護により**仕様上キャプチャできず**
> 黒く表示されます。これはプラグインの不具合ではなく、OS と再生アプリが協調した
> 著作権保護で、OBS の通常の画面/ウィンドウキャプチャでも同じく黒くなります。
> 保護を回避してキャプチャ可能にする手段は、技術的保護手段の回避にあたるため
> **提供できません**。保護されていない通常のアプリ・ウィンドウや、自分が権利を持つ
> ローカル動画は問題なくキャプチャできます。
...
ワイド画面の端を読みやすくしたい場合に使います。既定は 1.0m / 0.25x で、
50° FOV の機種では約 37 インチ相当を 1m 先に置いた、机上のモニターに近い
初期状態です。湾曲は OFF です。
`瞳孔間距離 IPD` は SBS 出力時の左右の眼の間隔（既定 63mm）です。自分の IPD に
合わせると、仮想スクリーンが設定したスクリーン距離に立体的に定位します
（距離が近いほど自然に寄り目になります）。SBS 出力が無効の間は影響しません。
...
メガネをかけたまま PC を操作するためのリモコンです。アプリのインストールは不要で、
同一 Wi-Fi 上のスマホ（Android / iPhone）のブラウザだけで動きます。

1. ドックの `スマホリモコン` セクションで `スマホからの操作を受け付ける` を ON にします。
   初回は Windows ファイアウォールの確認が出るので「アクセスを許可する」を選びます。
2. 表示された QR コードをスマホのカメラで読み取り、開いたページをそのまま使います。
   iPhone は `ホーム画面に追加` でフルスクリーン起動になります。Android は
   ページ右上の ⛶ ボタンで全画面表示にできます。スマホが接続されている間、
   ドックの QR コードは「リモコン接続中」表示に変わります（QR が無効になった
   わけではなく、接続が切れると再表示されます）。

ページの構成は、中央がタッチパッド（1 本指で移動、タップで左クリック、静止長押しで
ドラッグ開始（枠が緑の間は左ボタンを押したまま。指を離すと解放、一瞬で触り直せば
ドラッグ継続できるので長いドラッグも刻めます）、2 本指でスクロール、2 本指タップで
右クリック）、上部が `センター` / `再較正`、右端が `距離` ホイールです。
上部の ⚙ ボタンで設定画面が開き、ドックの各種設定（ステータス表示・
デバイス操作・SBS出力・音声の出力先・スクリーン距離/サイズ/湾曲など）を
スマホから直接変更できます。変更は即座にドックへ反映され、ドック側での
変更もスマホに追従します。
右端を下にスワイプすると**見ている場所に向かって**近づき、上にスワイプすると
離れます（視点が視線方向に移動するので、ワイドな wall の端の画面にも
「歩み寄って」読めます。スクリーン自体は動きません）。立ち位置は
`センター` を押すと元に戻ります。

操作の宛先は URL 内のランダムトークンで保護され、QR コードを読んだ端末以外からの
コマンドは拒否されます。トークンとポート（既定 8797）は設定に保存されるため、
一度読み取った QR は OBS を再起動しても使い続けられます。同時に操作できる
スマホは 1 台で、新しく接続した端末が優先されます。トークンを無効化したい
場合（家族に QR を見られた等）は、QR コードをクリックするか、接続中なら
`切断して QR を再発行` ボタンで再発行できます。ホテル Wi-Fi など端末間
通信が遮断されたネットワーク（AP アイソレーション）では届かないため、その場合は
スマホのテザリングに PC を接続してください。
