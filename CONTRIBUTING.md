...
## スマホリモコンの設定ミラー (remote_schema)

スマホリモコンの設定画面は、ドックの設定の一部を `src/remote_schema.cpp`
の定義表からミラーします。ページ（`data/remote.html`）は表のスナップ
ショットを受けてジェネリックに描画するだけで、個別の設定項目の知識を
持ちません。版ズレを防ぐため、次のルールを守ってください。

- **ドックに行（設定・ステータス表示）を追加・削除したら、リモコンに
  ミラーすべきか判断し、ミラーするなら `remote_schema.cpp` の表に
  1 エントリ追加・削除します。**ページ・プロトコル・JS の変更は不要です。
- ラベルはドックと同じロケールキーを参照します（文言の二重管理をしない）。
- 削除側の安全網: エントリは `g_device` のフィールドやセッターを直接参照
  するため、機能削除でそれらが消えればコンパイルエラーで気づけます。
- 「設定を既定値に戻す」と「スマホリモコン」セクションは意図的に
  ミラー対象外です（どちらもリモコン自身の接続を断ち切れるため）。
- 例外として、ページ自身の操作係数（カーソル速度等）は localStorage の
  ページローカル設定で、ミラーの対象外です（端末ごとに持つのが正しい
  値のため。ドックには出しません）。これらはページ内の `LOCAL_ROWS` で
  定義します。
- `data/remote.html` を変更したら、`<script>` 部を `node --check` で構文
  検証し、ファイルに制御文字（NUL 等）が混ざっていないことを確認して
  ください。script 内の NUL は HTML パーサーが U+FFFD に置換するため、
  ページは正常に見えるのに JS 全体が構文エラーで死にます（実例あり）。
  右下のリビジョン表示（r7 等）のバンプも忘れずに。
